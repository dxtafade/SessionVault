...
- Tests: `*.test.js` in this folder — run `npm test` (136 tests across the repo)
...
| `importData(blob, mode = 'merge')` | `{ imported, skipped }` | `mode`: `'merge'` \| `'replace'`; restores folders/smartFolders/spaces too; **validates & drops malformed entries** (incl. smart-folder rule shape) so a corrupt/hostile backup can't break the vault; **single atomic write** (safe on the sync-persist path) |
